...
// void MeshRenderer::setAttribute(ContainerBase *ga, int element) {
...
// void MeshRenderer::setAttribute(std::vector<float> attributeData) {
...
// void MeshRenderer::init(int nElements) {
...
// 	glBufferStorage(GL_TEXTURE_BUFFER, nElements * sizeof(float), nullptr, flags);
// 	// Map once and keep pointer (not compatible for MacOS... because need OpenGL >= 4.6 i think)
// 	ptrHighlight = (float*)glMapBufferRange(GL_TEXTURE_BUFFER, 0, nElements * sizeof(float), flags);
...
// 	glBufferStorage(GL_TEXTURE_BUFFER, nElements * sizeof(float), nullptr, flags);
// 	// Map once and keep pointer (not compatible for MacOS... because need OpenGL >= 4.6 i think)
// 	ptrFilter = (float*)glMapBufferRange(GL_TEXTURE_BUFFER, 0, nElements * sizeof(float), flags);
...
// void MeshRenderer::render(glm::vec3 &position) {
