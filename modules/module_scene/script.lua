...
function draw_tree(model, k, view)
	...
	if (imgui.TreeNode(k)) then 
		...
		draw_model_properties(model, k, app.scene.default_render_surface.view)
		...
		for k, child in pairs(app.scene.models) do
			...
			if (child.parent == model) then 
				draw_tree(child, k, app.scene.default_render_surface.view)
			end
		end
			...
			if (imgui.BeginListBox("##list_box_views")) then
				for k, _ in pairs(app.scene.views) do
					...
					if (imgui.Selectable(k .. "##list_box_selectable_view_" .. k, is_selected)) then
						local new_view = app.scene:get_view(k)
						app.scene.default_render_surface.view = new_view
					end
