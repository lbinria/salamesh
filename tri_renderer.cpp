...
void TriRenderer::setAttribute(ContainerBase *ga, int element) {
...
void TriRenderer::setAttribute(std::vector<float> attributeData) {
...
void TriRenderer::init() {
	...
	glBufferStorage(GL_TEXTURE_BUFFER, tri.nfacets() * sizeof(float), nullptr, flags);
	// Map once and keep pointer (not compatible for MacOS... because need OpenGL >= 4.6 i think)
	ptrHighlight = (float*)glMapBufferRange(GL_TEXTURE_BUFFER, 0, tri.nfacets() * sizeof(float), flags);
	...
	glBufferStorage(GL_TEXTURE_BUFFER, tri.nfacets() * sizeof(float), nullptr, flags);
	// Map once and keep pointer (not compatible for MacOS... because need OpenGL >= 4.6 i think)
	ptrFilter = (float*)glMapBufferRange(GL_TEXTURE_BUFFER, 0, tri.nfacets() * sizeof(float), flags);
...
void TriRenderer::push() {
	...
	nverts = tri.nfacets() * 3 /* 3 points per tri */;
	// std::vector<Vertex> vertices(nverts);

	// (8ms -> 3ms)
	std::vector<float> barys(tri.nfacets() * 3);

	const int size = tri.nfacets() / 3;
	for (int fi = 0; fi < size; ++fi) {
		// Compute bary
		const int off = fi * 3;
		const vec3 &v0 = tri.points[tri.facets[off]];	
		const vec3 &v1 = tri.points[tri.facets[off + 1]];
		const vec3 &v2 = tri.points[tri.facets[off + 2]];
...
void TriRenderer::render(glm::vec3 &position) {
