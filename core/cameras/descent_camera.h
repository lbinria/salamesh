...
//         float fov = getFov();
//         _proj = glm::perspective(glm::radians(fov), _screen.x / _screen.y, _nearPlane, _farPlane);
//     }

//     void lookAtBox(std::tuple<glm::vec3, glm::vec3> box) override {
//         auto [min, max] = box;
//         auto c = (min + max) * .5f;
//         _pos = {c.x, c.y, c.z + glm::length(max - min)};
//         _lookAt = c;
// 	    updateViewMatrix();
//     }
...
// 		glm::mat4 yawMat = glm::rotate(glm::mat4(1.0f), angleX, _up);
// 		glm::vec3 forwardY = glm::vec3(yawMat * glm::vec4(getViewDir(), 0.0f));

// 		// 3) Recompute right axis after yaw
// 		glm::vec3 rightAxis = glm::normalize(glm::cross(forwardY, _up));
...
// 		glm::vec3 newLookAt = _pos + (forwardYP * 1000.0f /* multiply is a quick & dirty fix. It enables look at to be very far away, therefore position doesn't collide with look at */);
...
// 		glm::mat4 view = glm::lookAt(_pos, newLookAt, _up);


//         // Update camera
//         setCameraView(_pos, newLookAt);
//     }
...
//         _pos += getRightVector() * speed;
//         _lookAt += getRightVector() * speed;
//         updateViewMatrix();
//     }
