...
void ApplyCustomStyle(const ImGuiTheme& theme) {
...
Image App::screenshot(const std::string& filename, int targetWidth, int targetHeight) {
	glBindFramebuffer(GL_READ_FRAMEBUFFER, scene.getCurrentView().getRenderSurface().fbo);
...
static void framebuffer_size_callback(GLFWwindow* window, int width, int height) {
	...
	app->getScene().getCurrentView().resize(width, height);
}
...
void App::start() {
	...
	ApplyCustomStyle(nordicSkyTheme);
	...
	while(!glfwWindowShouldClose(window)) {
		...
		scene.getCurrentView().getRenderSurface().bind();
		scene.getCurrentView().clear();
		...
		scene.getCurrentView().render(screenShader, quadVAO);
		...
		// ImGui::Begin("Render");
		// ImVec2 size = ImGui::GetWindowSize();
		// auto s = scene.getCurrentView().getRenderSurface();
...
void App::updateCamera(float dt) {
	...
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) {
		scene.getCurrentCamera().moveForward(speed);
	} else if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) {
		scene.getCurrentCamera().moveForward(-speed);
	}
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) {
		scene.getCurrentCamera().moveRight(-speed);
	} else if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) {
		scene.getCurrentCamera().moveRight(speed);
	}
	if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS) {
		scene.getCurrentCamera().moveUp(speed);
...
void App::key_event(int key, int scancode, int action, int mods) {
	...
	if (key == GLFW_KEY_LEFT_CONTROL && action == GLFW_PRESS) {
		scene.getCurrentCamera().setLock(true);
	} else if (key == GLFW_KEY_LEFT_CONTROL && action == GLFW_RELEASE) {
		scene.getCurrentCamera().setLock(false);
	}
